...
class NursingStation(Document):
	pass

def create_nursing_station_user(doc, method):
    ...
    if frappe.db.exists("User", doc.email):
        ...
        user = frappe.get_doc({
            "doctype": "User",
            "email": doc.email,
            "first_name": doc.nursing_station_name,
            "send_welcome_email": 0,
            "roles": [{"role": "Nursing Station"}],
    ...
    frappe.db.set_value("Nursing Station", doc.name, "user_id", user.name)